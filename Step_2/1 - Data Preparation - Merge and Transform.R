...
df_cs <- read.csv("source_data/All_Chargers.csv")
# Facilities around chargers, from OpenStreetMap:
poi_locations <- read.csv("source_data/facilities_around_coordinates.csv")
# Demographic information for every Dutch postal code, From CBS:
demog_data <- read_excel("source_data/CBS.xlsx")
# Distance to the closes highway, from Google maps:
highway_dist <- read.csv("source_data/highway_dist.csv")
# Auxiliary file:
...
unmatched_types_df <- distinct(data.frame(Type = unmatched_types)) # Convert unmatched_types to a data frame
write_xlsx(unmatched_types_df, "output/variables_to_check.xlsx") # Save this data frame to an Excel file
...
n_rating2_csv <- read.csv("source_data/n_rating.csv"); n_rating <- as.numeric(n_rating2_csv[[1]])
avg3_csv <- read.csv("source_data/avg3.csv"); avg3 <- as.numeric(avg3_csv[[1]]); avg3 <- round(avg3, 5)
avg5_csv <- read.csv("source_data/avg5.csv"); avg5 <- as.numeric(avg5_csv[[1]]); avg5 <- round(avg5, 5)
avg10_csv <- read.csv("source_data/avg10.csv"); avg10 <- as.numeric(avg10_csv[[1]]); avg10 <- round(avg10, 5)
...
# If the above loop is commented out, use this output:
cls_csv <- read.csv("source_data/cls.csv"); cls <- as.numeric(cls_csv[[1]])
...
df_all <- read.csv("source_data/All_Chargers.csv")
...
# If the above loop is commented out, use this output:
counts_1000_csv <- read.csv("source_data/counts_1000.csv"); counts_1000 <- as.numeric(counts_1000_csv[[1]])
counts_500_csv <- read.csv("source_data/counts_500.csv"); counts_500 <- as.numeric(counts_500_csv[[1]])
counts_250_csv <- read.csv("source_data/counts_250.csv"); counts_250 <- as.numeric(counts_250_csv[[1]])
counts_100_csv <- read.csv("source_data/counts_100.csv"); counts_100 <- as.numeric(counts_100_csv[[1]])
...
write_xlsx(reshaped_poi_locations, "output/reshaped_poi_locations.xlsx")
...
if (nrow(unmatched_types_df) > 0) {
  ...
  warning(paste("There are", nrow(distinct(unmatched_types_df)), "facility types that have not been categorized and will not be used in the model. They can be checked in the 'variables_to_check.xlsx' file. The last edit of the code was on", last_edit, "."))
}
